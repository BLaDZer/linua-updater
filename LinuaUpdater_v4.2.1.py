# ================================================================
#                       LINUA UPDATER v4.2.1
#           FINAL VERSION • By l1ntol • Enhanced Edition
# ================================================================
...
class SmartDownloader:
    ...
    def download(self, url, out_path, dlc_name=None, resume=False):
        ...
        success, msg = self._try_download_with_retry(url, out_path, temp_path, downloaded)
        if success:
            return True, "OK"
        ...
        for domain, mirror in mirrors.items():
            if domain in url:
                ...
                self.set_proxy(None)
                success, msg = self._try_download_with_retry(mirror_url, out_path, temp_path, downloaded)
                if success:
                    return True, "Downloaded via mirror"
        
        return False, "All download attempts failed"
    
    def _try_download_with_retry(self, url, out_path, temp_path, start_byte=0, max_retries=3):
        for attempt in range(max_retries):
            try:
                if attempt > 0:
                    delay = min(2 ** attempt, 10)
                    time.sleep(delay)
                success, msg = self._try_download(url, out_path, temp_path, start_byte)
                if success:
                    return True, "OK"
    ...
    def _try_download(self, url, out_path, temp_path, start_byte=0):
        try:
...
class SevenZipFinder:
    ...
    def find(self):
        ...
        try:
            ...
            pass
        self.logger.log("7-Zip not found", "WARNING")
        return None
...
class DLCSelector(QDialog):
    ...
    def populate(self, db, installed):
        ...
        available = [(dlc_id, info) for dlc_id, info in db.items() if dlc_id.upper() not in installed and dlc_id.upper() not in ["EP03", "EP06"]]
    ...
    def get(self):
        return [dlc for dlc, cb in self.cbs.items() if cb.isChecked() and cb.isEnabled() and dlc not in ["EP03", "EP06"]]
...
class LinuaUI(QMainWindow):
    ...
    def check_for_updates(self):
        ...
        checker.check_failed.connect(lambda err: None)
        QTimer.singleShot(0, checker.check_for_updates)
    ...
    def setup_ui(self):
        ...
        info = QLabel("EP03 City Living & EP06 Get Famous require MANUAL download!")
        info.setWordWrap(True)
        info.setStyleSheet("QLabel{background-color:#2a2a2a;padding:10px;border-radius:6px;color:white;font-size:11px;border-left:4px solid #ff9900;}")
    ...
    def update_dlc_status(self):
        ...
        available = [k for k in self.db.all().keys() if k.upper() not in installed and k not in ["EP03", "EP06"]]
        if len(available) == 0:
