...
SUPPORTED_ARCHIVE_EXTS_INTERNAL = ".zip"
SUPPORTED_ARCHIVE_EXTS_EXTERNAL = (".7z", ".001")
ARCHIVE_MULTIPART_DIGITS = 3
...
class SingleDLCInstaller:
    ...
    def run(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            if file_size < MIN_VALID_DOWNLOAD_SIZE:
    ...
    def run(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            for i, part in enumerate(parts):
                ...
                name = f"{self.dlc}_{threading.get_ident()}.7z.{str(i + 1).zfill(ARCHIVE_MULTIPART_DIGITS)}"
                out = os.path.join(tempfile.gettempdir(), name)
                dlc_name = f"{self.dlc} Part {i + 1}"
                part_weight = PERCENT_MAX / total_parts
                ...
                if self._progress_callback:
                    ...
                    def part_progress(
                        ...
                        total_progress = base + (progress * weight / PERCENT_MAX)
...
class TorrentInstaller:
    ...
    def run(self) -> Tuple[bool, Union[str, List[str]]]:
        ...
        try:
            ...
            if name.endswith(SUPPORTED_ARCHIVE_EXTS_INTERNAL):
                ok, reason = self.ex.extract_zip(primary, self.game)
            elif name.endswith(SUPPORTED_ARCHIVE_EXTS_EXTERNAL):
