...
def offline_database(tmp_path, monkeypatch):
    ...
    exercise download behavior override the stub per-test via an injected fake
    client (see ``tests/test_database.py``).
    """
    monkeypatch.setattr(AppPaths, "DATABASE_CACHE_FILE", tmp_path / "database_cache.json")
    monkeypatch.setattr(
        "linua_updater.core.database.HTTPClient",
        lambda **kwargs: FakeHTTPClient(404),
    )