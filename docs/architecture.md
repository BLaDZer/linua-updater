...
| `TorrentDownloader` | Engine-agnostic download service for magnet links: exposes the same `set_progress_callback`/`cancel`/`pause`/`resume`/`download` surface as `SmartDownloader`; orchestrates one injected `TorrentClient` (the pause/resume restart loop that kills the engine on pause and re-runs `--continue=true` on resume, progress-callback feed with dedupe, `.aria2`/`.torrent` artifact cleanup, and lifecycle logging). `is_available()` false → WARNING + `(False, "aria2c not found")`. No engine mechanics live here |
| `TorrentClient` | Abstract torrent-engine contract in `core/clients.py` (next to `HTTPClient`): `name`/`is_available`/`start`/`read_progress`/`stop`/`abort`/`wait_exit`. Implementations must be responsive to `stop()`/`abort()` from other threads (a blocked `read_progress()` must unblock). Adding a new engine = implement `TorrentClient` + one `TORRENT_CLIENTS` registry entry |
| `Aria2TorrentClient` | The sole `TorrentClient` implementation; owns everything aria2-specific: locating/verifying `aria2c`, building the command (the `ARIA2_FLAG_*` set, `--bt-stop-timeout`, `--continue=true`, `--summary-interval=1`), spawning the child with `CREATE_NO_WINDOW` on Windows via `_popen_kwargs()`, parsing summary lines (`_parse_size`/`_parse_summary`), and terminate/wait/kill reaping with an internal lock guarding the process pointer. Creates the output dir and raises on Popen failure/missing stdout so the service surfaces the error |
| `create_torrent_client(logger, client_name="aria2")` | External factory resolving a client name to a builder through the `TORRENT_CLIENTS` registry (`"aria2"` → `Aria2TorrentClient`); unknown names raise `ValueError`. The single extension point for new torrent engines (config-driven selection is a follow-up) |
...
| `Aria2Finder` | Locates the `aria2c` executable at runtime (PyInstaller `_MEIPASS` → exe dir → common paths → PATH); mirrors `SevenZipFinder`'s pattern. Consumed by `Aria2TorrentClient` |
...
1. Look up DLC metadata in `DLCDatabase` (typed `DLCInfo`). Install routing is source-based: the entry's `url`/`magnet`/`parts` and parsed `mirrors[]` are merged into one list sorted by `getPriority()` descending; the top item becomes `getMainDownloadSource()` and the rest `getMirrors()`. Default priorities are `magnet`=100, `parts`=50, `url`=30, so magnet wins by default. Each `DownloadSource` maps to an installer by `getType()`: `url` → `SingleDLCInstaller`, `parts` → `MultiPartInstaller`, `magnet` → `TorrentInstaller`. A failed source logs a `WARNING` and moves to the next mirror; a `Cancelled` result or `self._cancelled` aborts immediately with no fallthrough.
...
| `DLCInfo` / `DownloadSource` / `CheckSums` | Value objects in `core/models.py`. `DLCInfo` wraps an entry: the entry's sources (legacy `magnet`/`parts[]`/direct `url` and new `mirrors[]` entries) are merged, sorted stable by `getPriority()` descending, and the top item (if any) becomes `getMainDownloadSource()` — with default priorities `magnet`=100 > `parts`=50 > `url`=30 — while the rest become `getMirrors()`, each inheriting the entry-level `checksum` when it has none. `DownloadSource` carries a `type` (`url`/`parts`/`magnet`), its `source`/`parts`, per-source checksums, and an integer priority. `CheckSums` exposes `get("sha256")` etc. so `verify_file_checksums` works on it or on a plain dict |
...
5. **Run** — `InstallWorker` in a `QThread`; submits one future per DLC to a `ThreadPoolExecutor` (bounded by `Settings.max_threads`). Each DLC is routed by its `DownloadSource` type: `url` → `SingleDLCInstaller` (via `SmartDownloader`), `parts` → `MultiPartInstaller`, `magnet` → `TorrentInstaller` (via `aria2c`). Sources are tried in `getMainDownloadSource()` then `getMirrors()` order — highest priority first (default `magnet` > `parts` > `url`) — and a failed source falls back to the next mirror. All downloaders support pause/resume/cancel.
